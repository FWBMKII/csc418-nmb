...
void drawCircle(float r, int num, float z, int is_front)
{
    glBegin(GL_POLYGON);
    if(is_front == 1){
		glNormal3f(0,0,1);
...
void drawcy(float r1, int num, float z, float r2){
	glPushMatrix();
		...
		gluCylinder(IDquadric,r1,r2,z,num,num);
		drawCircle(r1,num,0.0, 0);
		drawCircle(r2,num,z, 1);
	glPopMatrix();
}
// Image functions
...
void display(void)
{
	...
	glPushMatrix();
		...
		if (renderStyle == OUTLINED){
			glDisable(GL_LIGHTING);
			glDisable(GL_LIGHT0);
			// draw the penguin with solid render
			glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
			glEnable(GL_POLYGON_OFFSET_FILL);
			glPolygonOffset(1.0f, 1.0f);
			glDisable(GL_POLYGON_OFFSET_FILL);
			drawPenguin(0);
			// draw the penguin again with wireframe render
			glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
			drawPenguin(1);
		}
		// matallic render
		if (renderStyle == METALLIC){
			...
			glDisable(GL_LIGHT0);
			glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
			//set light direction and enable lights
			GLfloat direction[] = {cos(LIGHT), sin(LIGHT), 0.0, 0.0};
			glLightfv(GL_LIGHT0, GL_POSITION, direction);
			glEnable(GL_LIGHTING);
			glEnable(GL_LIGHT0);
			//set material parameter of the penguin
			GLfloat ambientRGBA[] = {0.24725, 0.1995, 0.0745, 1};
			GLfloat diffuseRGBA[] = {0.75164, 0.60648, 0.22648, 1};
			GLfloat specularRGBA[] = {0.628281, 0.555802, 0.366065, 1};
		...
		if (renderStyle == MATTE){
			...
			glEnable(GL_LIGHT0);
			//set material parameter of the penguin
			GLfloat ambientRGBA[] = {0.02, 0.02, 0.02, .078125};
			GLfloat diffuseRGBA[] = {0.01, 0.01, 0.01, .078125};
			GLfloat specularRGBA[] = {0.4, 0.4, 0.4, .078125};
			glMaterialfv(GL_FRONT, GL_AMBIENT, ambientRGBA);
			glMaterialfv(GL_FRONT, GL_DIFFUSE, diffuseRGBA);
			glMaterialfv(GL_FRONT, GL_SPECULAR, specularRGBA);
			glMaterialf(GL_FRONT, GL_SHININESS, 3.0f);
...
void drawBody()
{
	glBegin(GL_QUADS);
		// draw front face
		glNormal3f(0, 0.1, 1);
		glVertex3f(-1.3, -3.0, 1.3);
		glVertex3f( 1.3, -3.0, 1.3);
		glVertex3f( 0.8,  2.0, 0.8);
		glVertex3f(-0.8,  2.0, 0.8);
...
void drawArm()
{
	glBegin(GL_QUADS);
		// draw front face
		glNormal3f(0, -0.05882352941176471, 1);
		glVertex3f(-0.0, -1.5, 0.3);
