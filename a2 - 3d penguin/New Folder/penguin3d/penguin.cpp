...
// Setting for light 
const float LIGHT_POSITION_MIN    = -180.0;
const float LIGHT_POSITION_MAX    =  180.0;
...
void initGlui()
{
	...
	// ***************************************************
        ...
        // Create control to set up the light source with xy position
        glui_panel = glui_render->add_panel("Light Source");
        glui_spinner = glui_render->add_spinner_to_panel(glui_panel, "Light angle", GLUI_SPINNER_FLOAT, &lightPosition);
        glui_spinner->set_float_limits(LIGHT_POSITION_MIN, LIGHT_POSITION_MAX, GLUI_LIMIT_WRAP);
        glui_spinner->set_speed(SPINNER_SPEED);
	//
	// ***************************************************
   ...
   // determine render style and set glPolygonMode appropriately
    switch (renderStyle) {
        case WIREFRAME:{
            ...
            break;}
        case SOLID:{
            // Draw white penguin for SOLID
           glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
            glColor3f(1.0,1.0,1.0);
            DrawPenguin();
            break;}
        case OUTLINED:{
            // Draw white penguin first
            renderStyle = SOLID;
            glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
            glColor3f(1.0,1.0,1.0);
            DrawPenguin();
            ...
            // change back to OUTLINED
            renderStyle = OUTLINED;
            break;}
        case MATTE:{
             //Change light to make it shinny or not
            glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
            double light_x = sin(lightPosition * (PI/180));
            double light_z = cos(lightPosition * (PI/180));
            GLfloat lightpos[] = {light_x , 1, light_z, 0};
            glLightfv(GL_LIGHT0, GL_POSITION, lightpos);
            glEnable(GL_LIGHTING);
            glEnable(GL_LIGHT0);
            GLfloat amb[] = {0, 0, 0, 0};
            GLfloat diff[] = {0.5, 0, 0, 1.0};
...
void DrawPenguin()
{
	glPushMatrix();
	    ...
	    glPushMatrix();
                ...
                glRotatef(joint_ui_data->getDOF(Keyframe::Keyframe::HEAD), 0.0, 1.0, 0.0);
                drawHead();
                drawtopBeak();
                glTranslatef(0.0f,-0.1f*joint_ui_data->getDOF(Keyframe::BEAK),0.0f);
                drawbotBeak();
            glPopMatrix();
	    ...
	    glPushMatrix();
                ...
                glRotatef(-joint_ui_data->getDOF(Keyframe::L_ELBOW), 1, 0, 0);
                drawlefthand();
            glPopMatrix();
	    ...
	    glPushMatrix();
                ...
                drawrightarm();
                glTranslatef(0.0f,-2.0f,0.0f);
                glRotatef(joint_ui_data->getDOF(Keyframe::R_ELBOW), 1, 0, 0);
                drawrighthand(); 
            glPopMatrix();
...
// Draw a the body, centered at the current location
void drawBody()
{
	glBegin(GL_QUADS);
		// draw top
...
// Draw the head, draw from the neck
void drawHead()
{
	glBegin(GL_QUADS);
		// draw top
...
// Draw the topbeak, consider the position of head.
void drawtopBeak()
{
	glBegin(GL_QUADS);
		// draw top
		...
		glVertex3f( -1.0,  1.0, 0.5);
		glVertex3f(-2.0,  1.0, 0.5);
		glVertex3f(-2.0,  0.7, 0.5);
		glVertex3f( -1.0,  0.7, 0.5);
                // draw back face
                glNormal3f( 0.0f, -1.0f, 0.0f);
            	glVertex3f(-2.0,  1.0, -0.5);
		glVertex3f( -1.0,  1.0, -0.5);
...
// Draw the botbeak, consider the position of head.
void drawbotBeak()
{
	glBegin(GL_QUADS);
		// draw top
...
// Draw the left arm, starting at shoulder
void drawleftarm()
{
        glBegin(GL_QUADS);
               //draw top
...
// Draw the left hand, starting at the end of left arm
void drawlefthand()
{
        glBegin(GL_QUADS);
               //draw top
...
// Draw the right arm, starting at shoulder
void drawrightarm()
{
        glBegin(GL_QUADS);
               //draw top
...
// Draw the right hand, starting at the end of right arm
void drawrighthand()
{
        glBegin(GL_QUADS);
               //draw top
...
// Draw the feet, starting at end of leg
void drawfeet()
{
        glBegin(GL_QUADS);
               //draw top
